    ...
    async with async_playwright() as p:
        ...
        if not matches:
            ...
            for index, match in enumerate(matches):
                ...
                captured_links = []
                # সরাসরি index.m3u8 (মাস্টার লিংক) ক্যাপচার করার লজিক
                ...
                try:
                    ...
                    # আপনার কথা অনুযায়ী পেজটিতে স্ট্রিম লোড হওয়ার জন্য পর্যাপ্ত সময় (১২ সেকেন্ড) অপেক্ষা করা হচ্ছে
                ...
                if master_link:
                    print(f"🟢 Captured Master Link (index.m3u8) for: {m_title}")
                    sub_file_content.append(f'#EXTINF:-1 tvg-logo="{m_logo}" group-title="FanCode",{m_title}')
                    sub_file_content.append("#EXT-X-VERSION:3")
                    
                    # ফ্যানকোডের মাস্টার লিংক থেকে বেস ইউআরএল বের করা
                    parsed_url = urllib.parse.urlparse(master_link)
                    base_path = parsed_url.path.rsplit('/', 1)[0] + '/'
                    query_string = f"?{parsed_url.query}" if parsed_url.query else ""
                    base_domain = f"{parsed_url.scheme}://{parsed_url.netloc}"
                    
                    # আপনার দেওয়া ফরম্যাট অনুযায়ী বিভিন্ন রেজোলিউশনের স্ট্রাকচার তৈরি করা
                    resolutions_data = [
                        {"res": "426x240", "name": "240p", "bandwidth": "446936", "codecs": "avc1.42e015,mp4a.40.2"},
                        {"res": "640x360", "name": "360p", "bandwidth": "702376", "codecs": "avc1.42e01e,mp4a.40.2"},
                        {"res": "854x480", "name": "480p", "bandwidth": "1023224", "codecs": "avc1.4d401e,mp4a.40.2"},
                        {"res": "960x540", "name": "540p", "bandwidth": "1278664", "codecs": "avc1.4d401f,mp4a.40.2"},
                        {"res": "1280x720", "name": "720p", "bandwidth": "1789512", "codecs": "avc1.64001f,mp4a.40.2"},
                        {"res": "1920x1080", "name": "1080p", "bandwidth": "3322120", "codecs": "avc1.640028,mp4a.40.2"}
                    ]
                    ...
                    for item in resolutions_data:
                        ...
                        sub_file_content.append(f"{base_domain}{base_path}index.m3u8{query_string}")
